...
class CliRunner {
  ...
  Future<int> run() async {
    ...
    try {
      ...
      if (results['version'] as bool) {
        print('unused_assets_scanner v0.1.0');
        return 0;
      }
  ...
  ArgParser _buildArgParser() {
    return ArgParser()
      ...
      ..addFlag('version', help: 'Show version number', negatable: false);
  }

  /// Build scan config from parsed arguments
  ScanConfig _buildConfig(ArgResults results) {
  ...
  Future<void> _outputJson(
    ...
    // Check for output file option from arguments
  ...
  String? _getOutputPath() {
    final parser = _buildArgParser();
    final results = parser.parse(arguments);
    return results['output'] as String?;
  }
...
Examples:
  ...
  dart run unused_assets

  # Scan specific project
  dart run unused_assets --path /path/to/project

  # Include test files
  dart run unused_assets --include-tests

  # Output as JSON
  dart run unused_assets --format json --output unused.json

  # Delete unused assets
  dart run unused_assets --delete

  # Exclude specific patterns
  dart run unused_assets --exclude "*.g.dart,**/generated/**"
''');
  }
}
